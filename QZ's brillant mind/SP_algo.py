...
# Creation of links between each created node.
w_graph = Graph.create_from_nodes([a,b,c,d,e,f])

# Specify the distance between each nodes.
...
# specify the current node to be reference to.
current_node = a

# Will show all the links relation between all the nodes affliated with the current node.
